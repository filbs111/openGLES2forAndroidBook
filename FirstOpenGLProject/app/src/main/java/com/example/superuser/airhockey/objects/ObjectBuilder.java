...
/**
 * Created by SuperUser on 25/09/2017.
 */
...
public class ObjectBuilder {
    ...
    private void appendCircle(Circle circle, int numPoints){
        ...
        for (int i = 0; i<= numPoints; i++){
            ...
            vertexData[offset++]= circle.center.x + circle.radius * (float) Math.cos(angleInRadians);   //apparently FloatMath doesn't have cos or sin anymore, so do something that might be wasteful instead
            vertexData[offset++]= circle.center.y;
    ...
    private void appendOpenCylinder(Cylinder cylinder, final int numPoints){
        ...
        final float yEnd = yStart + cylinder.height;
